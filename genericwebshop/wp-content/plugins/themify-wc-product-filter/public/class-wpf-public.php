...
class WPF_Public
{
	...
	private $custom_cols;
	private $main_query_key = 'themify_wpf_query_';
	...
	public function result_page() {
		if ( !empty( $_POST['id'] ) && !empty( $_POST['wpf'] ) ) {
			...
			delete_transient( $this->main_query_key);
        }
		if ( is_woocommerce() ) {
			add_action( 'woocommerce_before_shop_loop', array( $this, 'result_container' ), 100 );
			add_action( 'woocommerce_after_shop_loop', array( $this, 'close_div' ), 1 );
		}
	}
	...
	public function set_pagination() {
		...
		if ( $wp_query->max_num_pages > 1 && $wp_query->max_num_pages > $current_page ):
            ...
            <div class="wpf_infinity<?php if ( $this->pagination === 'infinity_auto' ): ?> wpf_infinity_auto<?php endif; ?>">
                <a data-max="<?php echo $wp_query->max_num_pages ?>" data-current="<?php echo( $current_page + 1 ) ?>"
	...
	public function get_result( array $data, array $form ) {
		...
		if ( !empty( $query_args ) ) {
		    ...
		    // add default params to query from main query
            $main_query_args = get_transient($this->main_query_key);
            if(false !== $main_query_args){
                foreach ($main_query_args as $arg_key=>$arg_val){
                    if(!empty($_POST[$arg_key]) || ('order' === $arg_key && !empty($_POST['orderby']))){
                        continue;
                    }
                    if(((!empty($main_query_args[$arg_key]) && empty($query_args[$arg_key])) || in_array($arg_key,array('order','orderby','posts_per_page'))) && !in_array($arg_key,array('no_found_rows','ignore_sticky_posts'))){
                        $query_args[$arg_key] = $arg_val;
                    }
                }
                $main_query_args = null;
            }
	...
	function override_wc_filter_query( $content ) {
	    if ( !empty( $_GET['id'] ) && !empty( $_GET['wpf'] ) ) {
			...
			add_filter( 'woocommerce_shortcode_products_query', array( $this,'save_original_wc_shortcode_query') );
			if(!empty($GLOBALS['themify']) && did_action('themify_header_after')){
				add_action( 'pre_get_posts', array( $this, 'save_original_wc_query' ) );
			}
		}
	...
	function save_original_wc_query( $query ) {
	    if('product' === $query->get( 'post_type' )){
			$this->main_query_key .= get_the_ID();
	        set_transient( $this->main_query_key, $query->query_vars ,HOUR_IN_SECONDS);
			remove_action( 'pre_get_posts', array( $this, 'save_original_wc_query' ) );
        }
	}
	...
	function change_shop_query($query){
	    if (is_shop() && $query->is_main_query() ) {
		    ...
		    remove_action( 'pre_get_posts', array( $this, 'change_shop_query' ),99 );
		}
    }

	function save_original_wc_shortcode_query( $args ) {
		$this->main_query_key .= get_the_ID();
		set_transient( $this->main_query_key, $args ,HOUR_IN_SECONDS);
		remove_filter( 'woocommerce_shortcode_products_query', array( $this,'save_original_wc_shortcode_query') );
		return $args;
	}
}
